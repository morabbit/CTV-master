#include "..\include\CasePlanTableWidget.h"

CasePlanTableWidget::CasePlanTableWidget(QWidget *parent)
	: QTableWidget(parent) {
...
CasePlanTableWidget::~CasePlanTableWidget() {
	delete this->menu;
	this->menu = NULL;
	this->sql = NULL;
}

void CasePlanTableWidget::SetDicomInfoTable(DicomInfoTable *sql) {
	this->sql = sql;
}

void CasePlanTableWidget::contextMenuEvent(QContextMenuEvent* event) {
	if (this->itemAt(event->pos()) != NULL) {
		this->menu->exec(QCursor::pos());
	}
}
void CasePlanTableWidget::Top() {
...
void CasePlanTableWidget::Bottom() {
	int columns = this->columnCount();
...
void CasePlanTableWidget::Delete() {
	this->sql->DeleteDate(this->currentItem()->text());
	this->removeRow(this->currentRow());
}

void CasePlanTableWidget::Clear() {
