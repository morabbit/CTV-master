#include "pch.h"
#include "..\include\CBImageFilter.h"

CBImageFilter::CBImageFilter()
{
}
...
CBImagePointer CBImageFilter::ReadImage(CPCHAR __path)
{
	__clear_var();
...
RGBCBPointer CBImageFilter::ReadRgbImage(CPCHAR __path)
{
	__clear_var();
...
CBImagePointer 
CBImageFilter::ReadImage(CPCHAR __path, UShortDicomVO & __dicom_vo)
{
	load_image __ld_img;
	ImageHeader _img_head;
	__file_path__ = __path;
	__parse_path();
	CBImagePointer __img = __read_sigle_file_with_head(&_img_head, &__ld_img, true);
...
void 
CBImageFilter::SaveImage(CBImagePointer __img_ptr,
...
CBImagePointer 
CBImageFilter::VtkImageToItkImage(vtkSmartPointer<vtkDataSet> __itk_img)
{
	__clear_var();
	CBImagePointer __img_ptr =  __VtkImageToItkImage(__itk_img, VTK_UNSIGNED_SHORT);
