...
#include "..\include\ReconditeImageFilter.h"
...
ImageFilter::ImageFilter(const ImageFilter & obj)
{
	*this = obj;
}

ImageFilter & ImageFilter::operator=(const ImageFilter & obj)
{
	this->__dims__ = obj.__dims__;
	this->__err_msg__ = _StrStm_();
	this->__err_msg__ << obj.__err_msg__.str();
	//this->__ext__ = obj.__ext__;
	this->__file_path__ = obj.__file_path__;
	if(obj.__ext__[0] != '\0')
	{
		memcpy(this->__ext__, obj.__ext__, ARRAYSIZE(obj.__ext__));
	}
	if(obj.__fname__[0] != '\0')
	{
		memcpy(this->__fname__, obj.__fname__, ARRAYSIZE(obj.__fname__));
	}
	
	return *this;
}

ImagePointer ImageFilter::ReadImage(CPCHAR __path)
{
	__clear_var();
...
RGBPointer ImageFilter::ReadRgbImage(CPCHAR __path)
{
	__clear_var();
...
ImagePointer ImageFilter::ReadImage(CPCHAR __path, ShortDicomVO & __dicom_vo)
{
	load_image __ld_img;
	ImageHeader _img_head;
	__file_path__ = __path;
	__parse_path();
	ImagePointer __img = __read_sigle_file_with_head(&_img_head, &__ld_img, true);
...
ImagePointer 
ImageFilter::ImportImage(PSHORT __buf, ImageHeader __hd, UINT & __bufLenth)
{
	return this->ImportBufferToImage(__buf, __hd, __bufLenth);
}

void 
ImageFilter::SaveImage(
	ImagePointer __img_ptr, CPCHAR __path, CPCHAR __name, CPCHAR __extent)
{
	__clear_var();
...
ImagePointer 
ImageFilter::VtkImageToItkImage(vtkSmartPointer<vtkDataSet> __itk_img)
{
	__clear_var();
	ImagePointer __img_ptr = __VtkImageToItkImage(__itk_img, VTK_SHORT);
